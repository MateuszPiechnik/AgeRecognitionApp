...
MODEL_SAVE_PATH = '/kaggle/working/best_hybrid_resnet_vit_age_model.pt'
...
common_resize_interpolation = transforms.InterpolationMode.BICUBIC

train_transforms = transforms.Compose([
    transforms.Resize((IMG_SIZE, IMG_SIZE), interpolation=common_resize_interpolation),
...
def plot_history(history):
    ...
    plt.title('Średni błąd bezwzględny (MAE) przy wykorzystaniu hybrydy Vision Transformer i ResNet50')
    plt.xlabel('Epoka')
    plt.ylabel('MAE')
    plt.xticks(ticks)
    plt.legend()
